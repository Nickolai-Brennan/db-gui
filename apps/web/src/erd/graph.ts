import type { PgSnapshot } from "../api/types";
import type { ErdAnnotations } from "../api/types";
...
export function buildGraph(snapshot: PgSnapshot, annotations?: ErdAnnotations): ErdGraph {
  const colsByTable = new Map<string, { name: string; dataType: string; isNullable: boolean }[]>();
  ...
  const idxByTable = new Map<
    string,
    { name: string; columns: string[]; isUnique: boolean; isPrimary: boolean; isValid: boolean }[]
  >();
  ...
  const badgeByTable = new Map<string, { severity: string; count: number }>();
  ...
  const nodes: ErdNode[] = snapshot.tables.map((t) => {
  ...
  const edgeAnn = new Map<string, { severity: string; count: number }>();
